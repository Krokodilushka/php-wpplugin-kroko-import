...
class XMLParser {
    ...
    static function parse($simpleXml) {
        if (!isset($simpleXml->post)) {
            throw new \KrokoImport\Exceptions\XML("посты не найдены");
        }
        $feed = new Feed();
        $itemPos = 0;
        foreach ($simpleXml->post as $post) {
            if (!isset($post->id)) {
                throw new \KrokoImport\Exceptions\XML("id не найден в post $itemPos");
            }
            if (!isset($post->title)) {
                throw new \KrokoImport\Exceptions\XML("title не найден в post $itemPos");
            }
            $id = $post->id;
            $title = $post->title;
            $slug = NULL;
            ...
            if (isset($post->category)) {
                ...
                foreach ($post->category as $value) {
                    if (!isset($value->id) || !isset($value->value)) {
                        throw new \KrokoImport\Exceptions\XML("в категории нет id или value. post ID $id, категория #" . $catPos);
                    }
            ...
            if (isset($post->meta)) {
                ...
                foreach ($post->meta as $value) {
                    if (!isset($value->key) || !isset($value->value)) {
                        throw new \KrokoImport\Exceptions\XML("в мета нет key или value. post ID $id, meta #" . $metaPos);
                    }
    ...
    static function processComments($comments, $simpleXMLElement) {
        if (isset($simpleXMLElement->comment)) {
            foreach ($simpleXMLElement->comment as $comment) {
                if (!isset($comment->id)) {
                    throw new \KrokoImport\Exceptions\XML("у комментария должен быть guid");
                }
                if (!isset($comment->author)) {
                    throw new \KrokoImport\Exceptions\XML("у комментария должен быть author. comment id" . $comment->id);
                }
                if (!isset($comment->date) || !is_numeric((string) $comment->date)) {
                    throw new \KrokoImport\Exceptions\XML("у комментария должен быть date и это должно быть число. comment id" . $comment->id);
                }
                if (!isset($comment->text)) {
                    throw new \KrokoImport\Exceptions\XML("у комментария должен быть text. comment id" . $comment->id);
                }
                $metas = new KeyValueStorage();
                // мета 
                if (isset($comment->meta)) {
                    ...
                    foreach ($comment->meta as $value) {
                        if (!isset($value->key) || !isset($value->value)) {
                            throw new \KrokoImport\Exceptions\XML("в мета комментария нет key или value. comment xml ID $comment->id, meta #" . $metaPos);
                        }
                        $metas->put(new KeyValue((string) $value->key, (string) $value->value));
                        $metaPos++;
                    }
