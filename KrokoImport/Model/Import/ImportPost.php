...
class ImportPost extends Loggable
{
    ...
    function __construct(array &$_logs, ImportCategory $importCategory)
    {
        parent::__construct($_logs);
        $this->_importCategory = $importCategory;
    }
    ...
    public function getWPPostByXMLPostId(string $xmlPostId): WP_Post
    {
        ...
        if (!empty($wpPost)) {
            ...
            throw new WpPostNotFoundException('Wp post by xml post ID ' . $xmlPostId . ' not found');
        }
    }

    public function insert(Post $xmlPost): int
    {
        $args = self::DEFAULT_POST_ARGS;
        $args['post_title'] = $xmlPost->getTitle();
        $args['post_content'] = $xmlPost->getContent();
        $args['post_date'] = $xmlPost->getDate()->format('Y-m-d H:i:s');
    ...
    public function setPostThumbnail(string $postId, string $url, ?string $description)
    {
        ...
        if (!set_post_thumbnail($postId, $attachmentID)) {
            throw new AttachmentException('Ошибка set_post_thumbnail $postId: ' . $postId . ', $attachmentID: ' . $attachmentID);
        }
    ...
    private function insertDb(array $args, Post $xmlPost): int
    {
        ...
        if ($xmlPost->getCategories()->count() > 0) {
            ...
            foreach ($xmlPost->getCategories()->get() as $category) {
                ...
                } catch (WpCategoryNotFoundException $e) {
