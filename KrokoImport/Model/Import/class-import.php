...
class Import
{
    ...
    {
        $xml = xml_Parser::load($feed_options->get_url());
        ...
        if (!empty($parsed->count_posts())) {
            ...
            foreach ($parsed->get_posts() as $xmlPost) {
                $isNewPost = $this->process_post($feed_options, $xmlPost);
                if ($isNewPost) {
                    $new_posts++;
                }
    ...
    private function process_post(Feed_Options $feed_options, Post $xml_post): ?bool
    {
        $res = null;
        $wp_post_id = null;
        ...
        try {
            ...
            if (!$feed_options->get_on_exists_update()) {
