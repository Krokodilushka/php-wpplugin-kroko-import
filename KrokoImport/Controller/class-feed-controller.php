...
class Feed_Controller extends Controller {
	public function create(): string {
		...
		return $this->getHolder()->getView()->get( 'view-feed-options', array(
			'alerts'         => [],
			'xmlUrl'         => $feedUrl,
			'title'          => 'Feed ' . date( 'd.m.Y H:i' ),
			'intervalMin'    => 5,
			'onExistsUpdate' => true,
			'feedData'       => XML_Parser::parse( XML_Parser::load( $feedUrl ) )
		) );
	}
	...
	public function showPosts(): string {
		...
		return $this->getHolder()->getView()->get( 'view-feed-posts', array(
			'feedData' => XML_Parser::parse( XML_Parser::load( $feed->getUrl() ) )
		) );
	}
}